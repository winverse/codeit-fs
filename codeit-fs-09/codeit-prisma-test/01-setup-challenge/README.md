...
- [ ] Prisma 프로젝트를 초기화하세요.
  ...
  npx prisma init
  ```
- [ ] 생성된 `prisma/schema.prisma` 파일을 확인하고 내용을 이해하세요.

  ```prisma
  // Prisma schema 파일입니다.
  // 데이터베이스 연결과 Prisma Client 설정을 정의합니다.
...
- [ ] `.env` 파일이 생성되었는지 확인하고, `DATABASE_URL`을 본인의 PostgreSQL 데이터베이스 URL로 설정하세요.
  ```bash
  # 예시: PostgreSQL 연결 문자열
  DATABASE_URL=
  `
...
- [ ] `src/server.js` 파일에 Prisma Client 연결 테스트 코드를 추가하세요.
...
- [ ] `http://localhost:3000/health` 접속 시 Prisma 연결 상태가 표시되나요?
- [ ] `npm run format` 실행 시 코드가 자동으로 정렬되나요?
- [ ] 프로젝트 구조가 가이드라인과 같이 잘 생성되었나요?
- [ ] `prisma/` 폴더와 `schema.prisma` 파일이 생성되었나요?
