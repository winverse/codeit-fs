...
## 챌린지 과제: SEO 최적화된 강의 목록 페이지 만들기

강의에서 배운 `Helmet`을 사용하여, 새롭게 추가된 "강의 목록" 페이지에 SEO에 최적화된 헤드 정보를 설정해봅시다.

`challenge` 폴더에 준비된 파일들을 `starter` 또는 `solution` 폴더에 덮어쓰거나, 해당 파일의 내용을 참고하여 실습을 시작하세요. `App.jsx`에 이미 새로운 페이지에 대한 라우트 설정이 추가되어 있습니다.

### 미션

1.  **`pages/CourseList/CourseListPage.jsx` 파일 열기**
    - `HomePage`와 `AboutPage`에서 했던 것과 동일한 방식으로, `Helmet` 컴포넌트를 사용하여 헤드 정보를 설정하세요.
    - `TODO` 주석을 참고하여 코드를 작성합니다.
    - 다음 정보들을 포함하세요:
      - `title`: "강의 목록 - React Router 학습"
      - `meta description`: "React Router 강의 목록을 확인하고 원하는 강의를 선택하세요."
      - `og:title`: "강의 목록 - React Router 학습"
      - `og:description`: "React Router 강의 목록을 확인하고 원하는 강의를 선택하세요."

### 확인하기

- 애플리케이션을 실행하고, 홈 페이지에서 "강의 목록 페이지로 이동" 링크를 클릭합니다.
- `/courses` 경로로 이동했을 때, 브라우저 탭의 제목이 "강의 목록 - React Router 학습"으로 올바르게 표시되면 성공입니다!
- 개발자 도구의 Elements 탭에서 `<head>` 섹션을 확인하여 메타 태그들이 올바르게 설정되었는지 확인해보세요.
