...
강의에서 배운 `useSearchParams`를 응용하여, 추천 검색어를 클릭했을 때 기존 정렬 상태는 유지하면서 새로운 검색을 실행하는 기능을 만들어봅시다.

`challenge` 폴더에 준비된 `pages/Search/SearchPage.jsx` 파일을 `starter` 또는 `solution` 폴더의 `src/pages/Search`에 덮어쓰거나, 해당 파일의 내용을 참고하여 실습을 시작하세요.

### 미션

1.  `pages/Search/SearchPage.jsx` 파일에 `handleClickSuggestion` 함수가 이미 선언되어 있습니다. 이 함수의 `TODO` 부분을 완성하세요.
2.  추천 검색어 버튼을 클릭하면 `handleClickSuggestion` 함수가 호출되고, 파라미터로 해당 검색어(suggestion)를 받습니다.
3.  `setSearchParams`를 호출하여 URL의 쿼리 파라미터를 업데이트해야 합니다.
4.  이때, 새로운 검색어(`q`)는 `suggestion`으로 설정하고, 기존의 정렬 상태(`sort`)는 그대로 유지해야 합니다. `searchParams.get('sort')`를 이용해 현재 정렬 값을 가져올 수 있습니다.

### 확인하기

1.  먼저 '오름차순 정렬' 버튼을 클릭하여 URL이 `/search?q=&sort=asc` 형태가 된 것을 확인합니다.
2.  '추천 검색어' 중 하나(예: 'React')를 클릭합니다.
3.  URL이 `/search?q=React&sort=asc`로 변경되고, 'React'에 대한 검색 결과가 오름차순으로 정렬되어 나타나면 성공입니다!
